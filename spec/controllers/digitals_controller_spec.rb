...
RSpec.describe Spree::DigitalsController, :type => :controller do
  ...
  context '#show' do
    ...
    context 'when exists attachment file' do
      let(:digital) { create(:digital, resource_url: nil) }
    ...
    context 'when not exist attachment file' do
      let(:digital) { create(:digital, attachment_content_type: nil, attachment_file_name: nil) }
      before { spree_get :show, secret: authorized_digital_link.secret }

      it { expect(response.redirect_url).to eq digital.resource_url }
    end
  end
end
